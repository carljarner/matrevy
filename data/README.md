...
| `program.json` | Medvirkende/Ordliste/QR-codes content for the printed programme booklet, edited on the Manus page's Program tab and rendered into `archive/<folder>/Program.pdf` |
...
```json
{
  "medvirkende": [
    {
      "id": "m1a2b3",
      "category": "Koordinatorer",
      "names": [
        { "id": "n1a2b3", "name": "Frida Nøhr Larsen", "note": "" },
        { "id": "n2c4d5", "name": "Carl Jarner", "note": "Boss" }
      ]
    }
  ],
  "ordliste": [
    { "id": "o1a2b3", "term": "Kridt", "definition": "Matematikerens yndlingsfallossymbol" }
  ],
  "qrCodes": [
...
- Feeds `archive/<folder>/Program.pdf`, the self-hosted printed audience programme booklet generated by `scripts/generate-pdfs.js`'s `buildProgramTex()` — replaces the old hand-maintained `program.tex` workflow. Edited exclusively via the Manus page's boss/admin-only **Program** tab (`server/update-data.php`'s `program` resource, boss level, `save_program`).
- `medvirkende` — one entry per named category (Koordinatorer, Band, ...); array order is print order. Each category's `names` is its own ordered array; `note` (optional) prints as `-- \emph{note}` after the name (e.g. a role/instrument), omitted entirely when empty.
- `ordliste` — term/definition pairs. `scripts/generate-pdfs.js` alphabetizes by `term` (Danish collation) at PDF-generation time regardless of the stored array order, so the editor's own order is for editing convenience only.
- `qrCodes` — `url` generates a QR code image at PDF-build time (via the `qrcode` npm package, `scripts/generate-pdfs.js`'s one other real dependency alongside `pdf-lib`) — no image is ever uploaded or stored; `label` is the heading printed above each QR code.
- Every free-text field (`category`, `name`, `note`, `term`, `definition`, `label`) is plain text typed into a web form and is **always** LaTeX-escaped when composed into Program.pdf — unlike `scenes.json`'s `name`/`scriptBody`/`melody`, which are raw pre-authored LaTeX inserted verbatim (see this file's `scenes.json` section and CLAUDE.md's Manus section).
...
- Program.pdf has no dedicated regeneration trigger — it rides the existing "Generér PDF'er" button/`generate-pdfs.yml` job alongside Aktoversigt/Rolleoversigt/Manuskript. If `data/program.json` is missing entirely, `scripts/generate-pdfs.js` skips Program.pdf generation with a log notice rather than failing the whole run.
